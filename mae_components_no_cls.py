"""
Version without Class Token
Including ShiftTransformer
"""
...
class MaskedViTEncoder(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        self.pos_embed=torch.tensor(get_2d_sincos_pos_embed(embed_dim, grid_size, cls_token=False, add_token=False)).unsqueeze(0) \
    ...
    def forward(self, x):
        ...
        x = x.mean(1) #avg on patch dim (excluding token)
        x = self.head(x) #use cls_token only for classification
...
class MaskedViTDecoder(nn.Module):
    def __init__(
        ...
        self.pos_embed=torch.tensor(get_2d_sincos_pos_embed(decoder_embed_dim, self.grid_size, cls_token=False, add_token=False)).unsqueeze(0).to(torch.float32) \
                        .to(self.device)
    ...
    def forward_decoder(self, x, mask, vis=False):
        # embed tokens
        x = self.encoder_to_decoder(x)

        x_clsfree=x #expired name
        ...
        x=self.decoder_pred(x) #deprive cls_token
...
class ShiftTransformer(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        self.pos_embed=torch.tensor(get_2d_sincos_pos_embed(embed_dim, self.grid_size, cls_token=False, add_token=False, shift_token=False)).unsqueeze(0) \
                                                .to(torch.float32).to(self.device)
        
        vit_config=args['ShiftTransformer']
        dpr = [x.item() for x in torch.linspace(0, vit_config['drop_path_rate'], vit_config['depth'])] # stochastic depth decay rule
        
        self.blocks = nn.ModuleList([
            Block(
                dim=vit_config['embed_dim'], num_heads=vit_config['num_heads'], mlp_ratio=vit_config['mlp_ratio'], qkv_bias=vit_config['qkv_bias'], qk_scale=vit_config['qk_scale'],
                drop=vit_config['drop_rate'], attn_drop=vit_config['attn_drop_rate'], drop_path=dpr[i], norm_layer=nn.LayerNorm,
                init_values=0)
            for i in range(vit_config['depth'])]) 
    ...
    def generate_shift(self, x):
        """Deprecated"""
        b = x.shape[0]
        embed_dim = x.shape[2]
 
        coord_x = np.expand_dims(np.random.randint(0, self.grid_size, b),(0,1)).repeat(embed_dim, 1).transpose(2,0,1)
        coord_y = np.expand_dims(np.random.randint(0, self.grid_size, b),(0,1)).repeat(embed_dim, 1).transpose(2,0,1)
        xy = np.concatenate([coord_x, coord_y], axis=1)

        return xy

    def generate_shift_cat(self, x):
        b = x.shape[0]
        embed_dim = x.shape[2]

        x = np.random.randint(0, self.grid_size, b)
        y = np.random.randint(0, self.grid_size, b)
        xy=torch.tensor(np.stack([x,y],axis=1)).to(self.device)

        return xy

    def augument_xy(self, xy):
        """receive 2D vector -> transform into embeddings"""
        xy_dot=xy[:,0]*self.grid_size + xy[:,1] #2D->1D
        xy_dot=torch.tile(xy_dot,(1, self.grid_size**2, 1)).transpose(0,2)
        #(b, 196, 1)
        
        return xy_dot
        
    def forward_encoder(self, x, xy_coord):
        """get token embeddings"""
        # embed patches
        batch_size = x.shape[0]
        ...
        # append shift token
        #xy = self.generate_shift(x)
        #shift_center = torch.tensor(xy, dtype=torch.float).to(self.device)
        #shift_token = shift_center + self.pos_embed[:, : self.grid_size**2, :].repeat(batch_size, 1, 1)
        shift_embed = xy.to(self.device)

        #x = torch.concatenate((shift_token, x), dim=1)
        x = torch.concatenate((shift_embed, x), dim=-1)
