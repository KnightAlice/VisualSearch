...
from models import Actor, Critic, Critic_vgg, ACMerge_vgg, ResNetLSTM
...
class Agent(nn.Module):
    ...
    def __init__(self,device,critic_path,grid_size=6,disable_critic=False,recog_threshold=0.5):
        super().__init__()
        self.grid_size=grid_size
        self.device=device
        self.disable_critic=disable_critic
        self.resnetlstm=ResNetLSTM().to(device)
    ...
    def forward(self,x,target_id,act_h0=None,act_c0=None,val_h0=None,val_c0=None):
        ...
        #ac merge
        #act_batch,state_values=self.acmerge(x)
        #vanilla actor
        #act_batch=self.actor(x)
        #resnet18
        act_h,act_c,val_h,val_c=self.resnetlstm(x,act_h0,act_c0,val_h0,val_c0)
