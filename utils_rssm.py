...
def select_action(obs_t, policy, sample_action=True, action_mask=None,
    ...
    #prev_state, prev_action for inner dynamics
    '''return grid index'''
    probs, values , epi_value, new_state = policy(*obs_t)
    if sample_action:
        ...
        return actions.view(-1), log_probs, values.view(-1), probs, epi_value, new_state
    else:
        raise NotImplementedError
        probs_new = probs.clone().detach()
...
def collect_trajs(env,
    ...
    obs=env.observe()
    act_batch, log_prob, value, prob, epi_value, prev_state = select_action((obs, env.target_id),
                                                    policy,
                                                    sample_action=True,
    ...
    if is_eval:
        ...
        while env.step_id < max_traj_length-1:
            act_batch, log_prob, value, prob, epi_value, prev_state = select_action((obs, env.target_id, prev_state, act_batch),
                                                    policy,
                                                    sample_action=True,
                                                    action_mask=None)#not implemented
